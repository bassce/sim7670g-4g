...
新增 [translations](translations/) 与 [pids](pids/) 用于后续静态下载服务；GitHub 为主仓库，国内 Gitea 可拉取镜像。已制作 250 条简体中文及配套英文语言包，提供 gzip、文件目录及校验值；网页语言加载和在线模板选择尚未实现。热点不暂停上传、可选蓝牙待机节电已记录方案但未实施；在线固件升级留待正常版本迭代。
