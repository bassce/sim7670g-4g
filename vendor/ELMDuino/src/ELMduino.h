...
public:
    ...
    bool prepareHeader(uint32_t header);
    bool prepareQuery(uint32_t header, const ELMQueryConfig& config);
    bool restoreHeader();
    uint64_t response = 0;
    uint16_t recBytes;
    uint8_t numPayChars;
    ...
    double processPID(const uint8_t& service, const uint16_t& pid, const uint8_t& num_responses, const uint8_t& numExpectedBytes, const double& scaleFactor = 1, const float& bias = 0, uint8_t dataOffset = 0, uint8_t dataLength = 0, bool signedValue = false);
...
private:
    bool numericResponse = false;
    bool recoveryRequired = false;
    bool pendingResponse = false;
    uint32_t requestStarted = 0;
    bool headerDirty = false;
    char activeProtocol = '0';
    char restoreProtocol = '0';
    bool queryConfigDirty = false;
    bool receiveFilterActive = false;
    bool resolveProtocol();
    bool applyHeader(uint32_t header);
    bool configureQueryCommand(const char* command);
    byte configuredDataTimeout = 0;
    bool recoverTransport();
    ...
    int32_t nextIndex(char const *str,
                      char const *target,
                      uint8_t     numOccur = 1);
